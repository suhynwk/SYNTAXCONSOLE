...
function touchMoved() {
  if (touches.length > 0) {
    ...
    // 움직임 계산 (Y축 드래그 -> 회전)
    // [중요] deltaY가 양수면 아래로, 음수면 위로 드래그
    let deltaY = currY - prevTouchY; 
    
    // 화면 하단부 (75% 아래) 터치 시 -> 크로스페이더 (가로 움직임)
    if (currY > height * 0.75) {
    ...
    // 화면 상단부 (75% 위) 터치 시 -> 원판 돌리기 (세로 움직임)
    else {
      // 화면 왼쪽 절반 -> Deck A
      if (currX < width / 2) {
        deckA.targetRotation += deltaY * 0.8; // 0.8은 회전 감도 (클수록 민감)
      } 
      // 화면 오른쪽 절반 -> Deck B
      else {
        deckB.targetRotation += deltaY * 0.8;
      }
    }
